...
class App extends Component {
  ...
  onClickAnyThumbnail = id => {
    const {timer, randomNumber} = this.state
  ...
  onClickPlayAgain = () => {
    this.setState({timer: 60, score: 0, isResult: false})
  }
  ...
  render() {
    const {score, timer, isResult, randomNumber, activeTabId} = this.state
    const imageUrl = imagesList[randomNumber]
    console.log(imageUrl)
    const filtersList = imagesList.filter(
